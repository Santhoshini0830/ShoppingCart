...
export const ProductDetails = ({}) => {
  ...
  const product =
    productList && productList.find((products) => products.id == productId.id);
  const {
    title,
    image,
    images,
  ...
  //     if (!item.id.includes(productId.id)) {
  //       item["id"].push(productId.id);
  //     }
  //     return;
  //   }
  //   return;
  ...
  recentProducts.forEach((item) => {
    if (item.email === loginUserEmail) {
      ...
      if (!item.ide.includes(productId.id)) {
        item.ide.push(productId.id);
      }
      return;
    }
  });
  ...
  localStorage.setItem("userData", JSON.stringify(recentProducts));
  const { id } = recentProducts.find((e) => e.email === loginUserEmail) || {};
  // const ele = loginUserEmail ? id.splice(-1) : "";

  const previouslyViewedProducts = id
    ? productList.filter((item) => id.includes(`${item.id}`))
    : [];

  const relatedTo = productList.filter(
