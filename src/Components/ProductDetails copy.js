...
export const ProductDetails = ({ list, cart }) => {
  ...
  // if (!product) {
  //   return <div>Loading...</div>; // Or any other loading or error UI
  // }

  // Destructure the properties only if the product is defined
  const {
    title,
    image,
    images,
  ...
  //     //   item["ide"].push(id);
  //     // }
  //     if (item.id && !item.id.includes(id)) {
  //       item["ide"].push(id);
  //     }
  //     return;
  //   }
  ...
  const previouslyViewedProducts = ide
    ...
    : [];
  console.log(previouslyViewedProducts, "details page ");
